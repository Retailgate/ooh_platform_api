    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (username) {
          ...
          //[username]);
          sqlCheckUser = SqlString.format(
            `SELECT "userName" FROM "users" WHERE "userName" = $1;`
          );
          paramsCheckUser = [username];
          var resCheckUser: any = await DBPG.query(
            sqlCheckUser,
            paramsCheckUser
          );
          if (!resCheckUser.length) {
            ...
            if (!resCheckUser.length) {
              resolve({
                token: null,
                error_message: "Account cannot be found",
              });
            }
        ...
        if (!result.length) {
          resolve({
            token: null,
            error_message: "You have entered a wrong password",
          });
        }
  ...
  verifyToken: async (req: any, res: any, next: any) => {
    try {
      ...
      if (key) {
        ...
        const match = keys.some((k: any) => bcrypt.compareSync(key, k.key));
        if (!match) {
        ...
        var sql =
          SqlString.format(`SELECT u."user_id", u."firstName", u."lastName", u."userName", u."emailAddress", p."password", u."role_id" 
                FROM "users" AS u 
